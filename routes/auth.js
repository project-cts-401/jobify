    ...
    const { registerUser, findUserByEmail, updateUserProfile } = require('../database/queries');
    ...
    router.post('/login', async(req, res) => {
        ...
        findUserByEmail(email, async(err, user) => {
            if (err || !user) return res.render('login', { user: null, error: 'Invalid credentials' });
            const valid = await bcrypt.compare(password, user.password);
            if (!valid) return res.render('login', { user: null, error: 'Invalid credentials' });
    ...
    router.post('/register', upload.fields([{ name: 'transcript' }, { name: 'resume' }, { name: 'id_doc' }]), async(req, res) => {
        try {
            ...
            registerUser({ student_number, name, surname, title, email, password: hashedPassword, role, gpa, skills, transcript_path, resume_path, id_path },
                (err) => {
                    if (err) return res.render('register', { user: null, error: 'User exists or invalid data' });
    ...
    router.get('/profile', requireAuth, (req, res) => {
        findUserByEmail(req.session.user.email, (err, user) => {
            if (err) return res.redirect('/auth/login');
            res.render('profile', { user: req.session.user, profile: user, error: null });
        });
    });
    ...
    router.post('/profile', requireAuth, upload.fields([{ name: 'transcript' }, { name: 'resume' }, { name: 'id_doc' }]), (req, res) => {
        ...
        updateUserProfile(req.session.user.id, { student_number, name, surname, title, gpa, skills, transcript_path, resume_path, id_path }, (err) => {
            if (err) return res.render('profile', { user: req.session.user, profile: req.session.user, error: 'Update failed' });
            res.redirect('/jobs');
        });
    });
